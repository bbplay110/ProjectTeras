...
public class FootIK : MonoBehaviour {
	...
	void Start () 
	{
		...
		myCollider =transform.parent.GetComponent<CharacterController>();
		//Guardar os valores
		//Save the values
	...
	{	
		...
		{	
			...
			{	
				...
				if(avatar.GetCurrentAnimatorStateInfo(0).IsName("GroundIdle") && myCollider.velocity.magnitude < 0.1f){
				...
				else if(avatar.GetCurrentAnimatorStateInfo(0).IsName("GroundIdle") || avatar.GetCurrentAnimatorStateInfo(0).IsName("walk 0")){
					//Definir o quanto vai afetar o [transform] do IK
					//Set how much will affect the IK [transform]
	...
	void IdleIK(){
		...
		if(Physics.Raycast(footPosL + Vector3.up, Vector3.down, out hit, 2.0f, rayLayer))
		{
			...
			avatar.SetIKRotation(AvatarIKGoal.LeftFoot, Quaternion.LookRotation(Vector3.ProjectOnPlane(hit.normal, footL.forward),  hit.normal));
        ...
        //[RayCast] para o chão, pra saber a distancia
        //[RayCast] to the ground, to know the distance
        Debug.DrawLine(footPosR + Vector3.up, Vector3.down * 2.0f, Color.blue, 0.1f);
		if(Physics.Raycast(footPosR + Vector3.up, Vector3.down, out hit, 2.0f, rayLayer))
		{
			...
			//Definir a nova rotação do IK
			//Set the new rotation of IK
			avatar.SetIKRotation(AvatarIKGoal.RightFoot,Quaternion.LookRotation(Vector3.ProjectOnPlane(hit.normal, footR.forward),  hit.normal));
			//Guardar a posição da colisão
			//Save the collision position
			footPosR = hit.point;
		}
	...
	{ 
		...
		if(ikActive){
			...
			//Se a situação do player for [Walk] ou [Run]
			//If the situation of the player is [Walk] or [Run]
			else if(avatar.GetCurrentAnimatorStateInfo(0).IsName("walk 0")){
	...
	void WalkRunUpdateCollider () 
	{
		...
		if(Physics.Raycast(transform.position + (((transform.forward * (myCollider.radius))) + (myCollider.velocity * Time.deltaTime)) + Vector3.up, Vector3.down, out hit, 2.0f, rayLayer))
		{
			//Mostrar o [Ray]
			//Show [Ray]
			Debug.DrawLine(transform.position + (((transform.forward * (myCollider.radius))) + (myCollider.velocity * Time.deltaTime)) + Vector3.up, hit.point, Color.red);
